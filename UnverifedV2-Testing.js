...
(function() {
      ...
      <div id="uv2-settings-body">
        ...
        <div id="uv2-settings-content">
          ...
          <div class="uv2-settings-page" id="uv2-page-about">
            ...
            <div class="uv2-setting-row"><div><div class="uv2-setting-label">Version</div><div class="uv2-setting-desc">2.36</div></div></div>
  ...
  function updateLanguage() {
    ...
    const moduleKeys = ['autoFullscreen','keystrokes','fpsCounter','cpsCounter','muteChat','pingCounter','fpsBooster','antiAfk','timeDisplay','musicPlayer','serverTranslator'];
  ...
  translatorModule.addEventListener("click", () => {
    ...
    if (isTranslatorActive) {
      async function detectAndTranslate(text) {
        try {
          ...
          return { translated: data[0].map(x => x[0]).join(''), detectedLang: data[2] };
        } catch {
          try {
            const res = await fetch(`https://api.mymemory.translated.net/get?q=${encodeURIComponent(text)}&langpair=auto|en`);
            const data = await res.json();
            return { translated: data.responseData.translatedText, detectedLang: 'unknown' };
      ...
      async function processElement(el) {
        ...
        btn.onclick = async (e) => {
          ...
          btn.remove();
          el.parentElement?.appendChild(unBtn);
        };
        el.parentElement?.appendChild(btn);
      }

      translatorModule._observer = new MutationObserver(mutations => {
        mutations.forEach(m => {
      ...
      translatorModule._observer.observe(document.body, { childList: true, subtree: true });
      document.querySelectorAll('*').forEach(el => { if (el.children.length === 0) processElement(el); });

    } else {
      if (translatorModule._observer) {
        translatorModule._observer.disconnect();
        translatorModule._observer = null;
      }
      document.querySelectorAll('[data-uv-translate]').forEach(el => {
        delete el.dataset.uvTranslate;
      });
    }
  });
